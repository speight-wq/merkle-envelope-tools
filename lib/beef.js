...
(function (root, factory) {
  ...
  // ---- BRC-95 Atomic BEEF subject validation -----------------------
  // An Atomic BEEF names a subject txid in its prefix. BRC-95 requires that the
  // subject is the LAST transaction in the container and that the container holds
  // only the subject and its ancestors. Without this, `atomicSubject` is an
  // unvalidated attacker-controlled claim: any txid (even one absent from the
  // container, or one the enclosed proofs don't cover) could be asserted as "the
  // transaction this Atomic BEEF proves" (audit M1). Fail closed on any violation.
  function validateAtomicSubject(txs, subjectTxid) {
    if (!txs || !txs.length) throw new Error('Atomic BEEF: container has no transactions');
    var byId = {};
    txs.forEach(function (t) { byId[t.txid] = t; });
    // (1) subject must be present in the container
    if (!byId[subjectTxid]) {
      throw new Error('Atomic BEEF: subject txid ' + subjectTxid + ' is not present in the container');
    }
    // (2) subject must be the last transaction (BRC-95)
    if (txs[txs.length - 1].txid !== subjectTxid) {
      throw new Error('Atomic BEEF: subject txid must be the last transaction (BRC-95)');
    }
    // (3) container must contain ONLY the subject and its ancestors (BRC-95).
    // Reachable set = subject + everything reachable by following inputs to parents
    // that are themselves present in the container. Any included tx not in that set
    // is unrelated to the subject and must be rejected.
    var reachable = {}; reachable[subjectTxid] = true;
    var stack = [subjectTxid];
    while (stack.length) {
      var cur = byId[stack.pop()];
      if (!cur) continue;
      (cur.inputs || []).forEach(function (inp) {
        var p = inp.prevTxid;
        if (byId[p] && !reachable[p]) { reachable[p] = true; stack.push(p); }
      });
    }
    txs.forEach(function (t) {
      if (!reachable[t.txid]) {
        throw new Error('Atomic BEEF: transaction ' + t.txid + ' is not the subject or an ancestor of it (BRC-95)');
      }
    });
  }
  ...
  function parse(hex) {
    ...
    // BRC-95: validate the atomic subject binding (audit M1) — fail closed.
  ...
  return {
    ...
    isAtomic: isAtomic,
    validateAtomicSubject: validateAtomicSubject,
