...
(function(global) {
  ...
  function verifyHeaderChain(bytes, expectedCheckpoint, options) {
    ...
    // Difficulty floor enforced on the TIP (audit FP-1/FP-2, revised for finding D).
    // A forged low-difficulty chain cannot be admitted, because to place a forged
    // header anywhere in the chain the attacker must re-forge every subsequent header
    // (linkage), so the tip is forged and cheap too — and requiring the tip to clear
    // the static checkpoint floor forces real proof-of-work an attacker without
    // hashpower cannot produce. Enforcing on the tip (rather than every header) avoids
    // rejecting LEGITIMATE chains that contain a genuine intermediate difficulty drop
    // >8x, which a flat per-header floor would have falsely rejected.
    // NOTE: the floor is a heuristic (see THREAT-MODEL), and its strength depends on
    // CHECKPOINT.nBits being a real, current difficulty. Verify the checkpoint value.
    if (headers.length > 0) {
      const tip = headers[headers.length - 1];
      if (tip.target > STATIC_FLOOR_TARGET) {
        throw new Error('Chain tip at ' + tip.height + ' below difficulty floor ' +
