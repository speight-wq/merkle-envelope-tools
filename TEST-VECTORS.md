...
## Mainnet Transaction Vectors

In addition to cryptographic primitives, `lib/mainnet-vectors.js` contains real BSV mainnet transactions for end-to-end SPV verification testing.
...
| Block Hash | `000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f` |
| Merkle Root | Same as TXID (single-transaction block) |

Contains the famous coinbase message: "The Times 03/Jan/2009 Chancellor on brink of second bailout for banks"
...
| PrevBlock | Genesis block hash |

Verifies chain linkage from Block 1 → Block 0.
...
| Merkle Proof | 1 node (2-transaction block) |

This block contains the famous first Bitcoin transaction between two people: Satoshi Nakamoto sent 10 BTC to Hal Finney.

**Note on Merkle proof format:** Sibling hashes in proofs are stored in **internal byte order** (little-endian), not display format. This matches the `verifyMerkleProof` function in `lib/headers.js`.

### Cross-Verification

See `docs/CROSS-VERIFICATION.md` for instructions to independently verify each vector using:
- Block explorers (WhatsOnChain, Blockchair)
- Local computation (Python hashlib)
- BSV node RPC commands
